module.exports = function(grunt) {
    ...
    grunt
            .initConfig({
                ...
                copy : {
                    ...
                    resources : {
                        src : [ 'm.html', 'lib/jquery.mobile-1.4.0.css',
                                'lib/images/*', 'images/*' ],
                        dest : 'dist/'
                    },
                    locales : {
                ...
                jshint : {
                    sources : {
                        options : {
                            // -W032: eclipse requires some semicolons that
                            // jshint would
                            // not accept by default
                            // so we tell that they should be ignored here.
                            '-W032' : true,
                            // -W014: The eclipse formatter wraps expressions
                            // with the operator first in line but jshint does
                            // not like it.
                            '-W014' : true,
                        },
                        src : srcFileArray
    ...
    grunt.registerTask('default', [ 'uglify', 'copy', 'firefoxManifest',
            'compress' ]);
    // for development: check the source, create a uncompressed version and
    // copy it to the dist
    grunt.registerTask('debug', [ 'jshint', 'nodeunit', 'concat', 'copy',
            'firefoxManifest', 'compress' ]);
